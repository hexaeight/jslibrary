...
HexaEightCaptcha.prototype = (function() {
	...
	var bearertoken ="";
	    ...
	    if (aleconfig == "Default") {
		    var dstring = utf8ToHex(code).substring(0,5);
		    var diterations = parseInt(dstring,16);
		    defaultkeyBytes = CryptoJS.PBKDF2(code, CryptoJS.enc.Utf8.parse(code + code.reverse()), { hasher: CryptoJS.algo.SHA512, keySize: 48 / 4, iterations: diterations });
		    if (codechallenge != "") {
	    ...
	    if (aleconfig == "Quick") {
		    var dstring = utf8ToHex(code).substring(0,3);
		    var diterations = parseInt(dstring,16);
		    quickkeyBytes = CryptoJS.PBKDF2(code, CryptoJS.enc.Utf8.parse(code + code.reverse()), { hasher: CryptoJS.algo.SHA512, keySize: 48 / 4, iterations: diterations });
		    if (codechallenge != "") {
	        	    var fsecret = codeverifier + code + codeverifier;
			    var fstring = utf8ToHex(fsecret).substring(0,3);
			    var fiterations = parseInt(fstring,16);
	...
	function FetchQuickEncryptedCodeVerifier() {
		return AES_Quick_Encrypt(email.trim() + "|" + codeverifier.toString());			
	}
	...
	function AES_Default_Encrypt(data) {
		...
		return CryptoJS.AES.encrypt(CryptoJS.enc.Utf8.parse(data), key, { iv: iv, mode: CryptoJS.mode.CBC });
	}

	function AES_Default_Decrypt(encrypteddata) {
		var key = new CryptoJS.lib.WordArray.init(defaultkeyBytes.words, 32);
		var iv = new CryptoJS.lib.WordArray.init(defaultkeyBytes.words.splice(32 / 4), 16);
		return CryptoJS.AES.decrypt(encrypteddata, key, { iv: iv, mode: CryptoJS.mode.CBC });
	}
	...
	function AES_Quick_Decrypt(encrypteddata) {
		var key = new CryptoJS.lib.WordArray.init(quickkeyBytes.words, 32);
		var iv = new CryptoJS.lib.WordArray.init(quickkeyBytes.words.splice(32 / 4), 16);
		return CryptoJS.AES.decrypt(encrypteddata, key, { iv: iv, mode: CryptoJS.mode.CBC });
	}

	function AES_Quick_Encrypt(data) {
		var key = new CryptoJS.lib.WordArray.init(quickkeyBytes.words, 32);
		var iv = new CryptoJS.lib.WordArray.init(quickkeyBytes.words.splice(32 / 4), 16);
		return CryptoJS.AES.encrypt(CryptoJS.enc.Utf8.parse(data), key, { iv: iv, mode: CryptoJS.mode.CBC });
	}
